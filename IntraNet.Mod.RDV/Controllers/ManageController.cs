...
namespace IntraNet.Mod.RDV.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public string UserAvatar()
        {
            ...
            if (User.Identity.GetUserName() != null && User.Identity.GetUserName().IsEmpty()!=true)
            {
                string user = User.Identity.GetUserName();
                ApplicationUser users = db_.Users.Where(i => i.UserName == user).SingleOrDefault();
                if (users.ImagePath == null || users.ImagePath.IsEmpty())
                {
        ...
        public ActionResult InserirAvatar()
        {
            return View();
        }
        [HttpPost]
        public JsonResult InserirAvatarUser(AvatarUserViewModel model)
        {
            ApplicationDbContext db_ = new ApplicationDbContext();
            var message = "";
            string user = User.Identity.GetUserName();
            ...
            try
            {
                string fileName = Path.GetFileNameWithoutExtension(model.ImagePath);
                string extension = Path.GetExtension(model.ImagePath);
                fileName = user+"_"+ extension;
                users.ImagePath = "/Images/Avatar/" + fileName;
                fileName = HttpContext.Server.MapPath(@"~/Images/Avatar/"+ fileName);
                model.ImageFile.SaveAs(fileName);
                db_.Entry(users).State = EntityState.Detached;
                db_.Set<ApplicationUser>().AddOrUpdate(users);
                //db_.Entry(users).State = EntityState.Modified;
                db_.SaveChangesAsync();
                message = "Sua imagem foi inserida com sucessso!";
